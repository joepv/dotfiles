...
local function mode_icon()
	...
	local modes = {
		n = " \u{e7c5} NORMAL", -- nf-fa-apple {f179}
		i = " \u{f040} INSERT", -- nf-linux-vim
	...
	vim.api.nvim_create_autocmd({ "WinEnter", "BufEnter" }, {
		callback = function()
			vim.opt_local.statusline = table.concat({
				...
				--"%{v:lua.file_type()}",
				"%{v:lua.file_type()}%{exists('$VIRTUAL_ENV_PROMPT') ? ' (' . $VIRTUAL_ENV_PROMPT . ')' : ''}",
				"\u{e0b1} ", -- nf-pl-left_hard_divider
				"%{v:lua.file_size()}",
				"%=", -- Right-align everything after this
				" \u{ebd0} %l:%c  %P ", -- nf-fa-clock_o for line/col
			})
		end,
	})
